...
class TrainerRoad:
    ...
    def connect(self):
        ...
        if r.status_code != 200:
            # There was an error
            raise RuntimeError("Error loging in to TrainerRoad")

    def disconnect(self):
        r = self._session.get(self._logout_url)
        if r.status_code != 200:
            raise RuntimeError("Error loging out of TrainerRoad")
    ...
    def _get(self, url):
        ...
        if r.status_code != 200:
            raise RuntimeError("Error getting info from TrainerRoad")
    ...
    def _post(self, url, data):
        ...
        if r.status_code != 200:
            raise RuntimeError("Error posting info to TrainerRoad")
